...
class sfda_drug():
    def __init__(self):
        ...
        self.max_page = 3230  # 药品的总页数
        self.max_item = 48436  # 药品的总个数
        self.table_id = 27  # 进口器械
        ...
        # 数据库参数
        self.status_table = "sfda_instrument_entrance_id_status"
        self.drug_table = "sfda_instrument_entrance"
        ...
        logging.basicConfig(filename='sfda_instrument_entrance.log') # 日志输出文件
    ...
    def update_drug_data(self, drug, drug_id):
        sql_drug = """insert into {} values (0 , {}, "{}","{}","{}","{}","{}","{}",
            ...
            MySQLdb.escape_string(drug[u"产品名称"]), 
            MySQLdb.escape_string(drug[u"注册证编号"]), 
            MySQLdb.escape_string(drug[u"注册人名称"]), 
            MySQLdb.escape_string(drug[u"注册人住所"]),
            MySQLdb.escape_string(drug[u"生产地址"]), 
            MySQLdb.escape_string(drug[u"代理人名称"]), 
            MySQLdb.escape_string(drug[u"代理人住所"]), 
            MySQLdb.escape_string(drug[u"型号、规格"]), 
            MySQLdb.escape_string(drug[u"结构及组成"]), 
            MySQLdb.escape_string(drug[u"适用范围"]), 
            MySQLdb.escape_string(drug[u"生产国或地区（英文）"]),
            MySQLdb.escape_string(drug[u"附件"]), 
            MySQLdb.escape_string(drug[u"其他内容"]), 
            MySQLdb.escape_string(drug[u"备注"]), 
            MySQLdb.escape_string(drug[u"批准日期"]), 
            MySQLdb.escape_string(drug[u"有效期至"]), 
            MySQLdb.escape_string(drug[u"生产厂商名称（中文）"]),
            MySQLdb.escape_string(drug[u"产品名称（中文）"]), 
            MySQLdb.escape_string(drug[u"产品标准"]), 
            MySQLdb.escape_string(drug[u"生产国或地区（中文）"]), 
            MySQLdb.escape_string(drug[u"售后服务机构"]), 
            MySQLdb.escape_string(drug[u"变更日期"]), 
            MySQLdb.escape_string(drug[u"主要组成成分（体外诊断试剂）"]),
            MySQLdb.escape_string(drug[u"预期用途（体外诊断试剂）"]), 
            MySQLdb.escape_string(drug[u"产品储存条件及有效期（体外诊断试剂）"]), 
            MySQLdb.escape_string(drug[u"审批部门"]), 
            MySQLdb.escape_string(drug[u"变更情况"]), 
