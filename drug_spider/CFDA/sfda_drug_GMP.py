...
class sfda_drug():
    ...
    def __init__(self):
        ...
        self.max_page = 1924  # 进口药品的总页数
        self.max_item = 28860  # 进口药品的总个数
        self.table_id = 23  # GMP
        ...
        # 数据库参数
        self.status_table = "sfda_GMP_id_status"
        self.drug_table = "sfda_GMP"
        # requests_data
        ...
        self.loger = logging.basicConfig(filename='sfda_drug_GMP.log')
    ...
    def get_drug_id(self):
        while self.cur_page <= self.max_page:
            ...
            try:
                ...
                driver.close()
                self.cur_page += 1
                self.req_sleep("normal", 0.5)
            except Exception as e:
                print "--异常信息为:--\n%s" % e
                self.req_sleep("normal", 2)
                driver.quit()
    ...
    def update_sql(self, drug, drug_id):
        ...
        '{}', '{}', '{}', '{}', '{}')""".format(
            self.drug_table, drug_id, drug[u"省市"], drug[u"证书编号"], drug[u"企业名称"], drug[u"地址"],
            drug[u"认证范围"], drug[u"发证日期"], drug[u"有效期截止日"], drug[u"批准延续日期"],
            drug[u"有效期延续至"], drug[u"批准延续的认证范围"],drug[u"认证GMP版本"], drug[u"备注"], drug[u"注"]
        )
